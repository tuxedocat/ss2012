...
class PrepChecker(object):
    ...
    def __init__(self, path):
        ...
        self.gold_words = [dic["gold_words"] for dic in self.corpus]
        self.test_words = [dic["test_words"] for dic in self.corpus]
        self.corr_pairs = [dic["correctionpair"] for dic in self.corpus]
    ...
    def train(self, numexamples = 1400, numiter = 20):
        trainset_ppindexlist = self.ppindexlist[:numexamples]
        trainset_features = self.makefeatures(self.gold_words[:numexamples], trainset_ppindexlist)
    ...
    def test(self):
        ...
        classifier_outputs = []
        ns = self.numexamples
        testset_ppindexlist = self.ppindexlist[ns:]
        testset_features = self.makefeatures(self.test_words[ns:],testset_ppindexlist)
...
def main():
    ...
    pc.train(numexamples=1400, numiter=20)
    print "done!"
    print "Testing..."
    pc.test()
    et = time.time()
